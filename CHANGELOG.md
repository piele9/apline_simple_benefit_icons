...
- Configurable display location: product reassurance area, left/right column,
  product footer — or anywhere via `{widget name='apline_simple_benefits'}`.
...
- folder / class / `$this->name`: `apline_simple_<feature>`
- DB table: `<abbrev>_item` (here: `asb_item`)
- Configuration keys: `<ABBREV>_*` (here: `ASB_*`)
